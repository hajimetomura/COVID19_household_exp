...
data {
  ...
  // Weather date to exclude: int W_TT; // From 2020 Jan. 1.
  // Weather date to exclude: vector [W_TT] W_abs_hum; // The weighted average of indicators for absolute humidity.
  int TT_diff_M_H;
  // Weather date to exclude: int TT_diff_M_W; // Equal to TT_diff_M_H.
...
transformed data{
  ...
  for (t in 1:TT){
    ...
    // Weather date to exclude: EXPVAR[2,t] = dot_product(W_abs_hum[(TT_diff_M_W + 1 + t):(TT_diff_M_W + 1 + t + 13)], dist_incub);
    EXPVAR[2:10,t] = append_row(H_expvals[,(TT_diff_M_H + 1 + t):(TT_diff_M_H + 1 + t + 13)], M_trans[(1+t):(1+t+13)]) * dist_incub;
    // Weather date to exclude: EXPVAR[11:19,t] = append_row(H_expvals[,(TT_diff_M_H + 1 + t):(TT_diff_M_H + 1 + t + 13)], M_trans[(1+t):(1+t+13)]) * (dist_incub .* W_abs_hum[(TT_diff_M_W + 1 + t):(TT_diff_M_W + 1 + t +13)]);
    EXPVAR[11,t] = dot_product(D_SE1[(1+t):(1+t+13)], dist_incub);
    EXPVAR[12,t] = dot_product(D_SE2[(1+t):(1+t+13)], dist_incub);
    EXPVAR[13,t] = dot_product(D_pre_SE1[(1+t):(1+t+13)], dist_incub);
    EXPVAR[14:22,t] = append_row(H_expvals[,(TT_diff_M_H + 1 + t):(TT_diff_M_H + 1 + t + 13)], M_trans[(1+t):(1+t+13)]) * (dist_incub .* D_SE1[(1+t):(1+t+13)]);
    EXPVAR[23:31,t] = append_row(H_expvals[,(TT_diff_M_H + 1 + t):(TT_diff_M_H + 1 + t + 13)], M_trans[(1+t):(1+t+13)]) * (dist_incub .* D_SE2[(1+t):(1+t+13)]);
    EXPVAR[32:40,t] = append_row(H_expvals[,(TT_diff_M_H + 1 + t):(TT_diff_M_H + 1 + t + 13)], M_trans[(1+t):(1+t+13)]) * (dist_incub .* D_pre_SE1[(1+t):(1+t+13)]);
  }
...
parameters {
  ...
  // Weather date to exclude: real <upper=0> coef_AH; // Coefficient for the level effect of absolute humidity. Restricted to be negative, given the existing evidence of medical research.
  ...
  // Coefficient for the cross effect between household expeduture items or mobility in transportation, and the first state of emergency.
  real <lower=-scl1> scl_AH_SE11;
  real <lower=-scl2> scl_AH_SE12;
  real <lower=-scl3> scl_AH_SE13;
  real <lower=-scl4> scl_AH_SE14;
  real <lower=-scl5> scl_AH_SE15;
  real <lower=-scl6> scl_AH_SE16;
  real <lower=-scl7> scl_AH_SE17;
  real <lower=-scl8> scl_AH_SE18;
  real <lower=-scl9> scl_AH_SE19;

  // Coefficient for the cross effect between household expeduture items or mobility in transportation. Restricted to be negative, given the existing evidence of medical research.
  real <lower=-scl1> scl_AH_SE21;
  real <lower=-scl2> scl_AH_SE22;
  real <lower=-scl3> scl_AH_SE23;
  real <lower=-scl4> scl_AH_SE24;
  real <lower=-scl5> scl_AH_SE25;
  real <lower=-scl6> scl_AH_SE26;
  real <lower=-scl7> scl_AH_SE27;
  real <lower=-scl8> scl_AH_SE28;
  real <lower=-scl9> scl_AH_SE29;

  // Coefficient for the cross effect between household expeduture items or mobility in transportation. Restricted to be negative, given the existing evidence of medical research.
  real <lower=-scl1> scl_AH_pre_SE11;
  real <lower=-scl2> scl_AH_pre_SE12;
  real <lower=-scl3> scl_AH_pre_SE13;
  real <lower=-scl4> scl_AH_pre_SE14;
  real <lower=-scl5> scl_AH_pre_SE15;
  real <lower=-scl6> scl_AH_pre_SE16;
  real <lower=-scl7> scl_AH_pre_SE17;
  real <lower=-scl8> scl_AH_pre_SE18;
  real <lower=-scl9> scl_AH_pre_SE19;
...
transformed parameters {
  ...
  // Weather date to exclude: row_vector [9] scl_AH; // row vector of scl_AHi for i = 1,2,...10.
  row_vector [9] scl_AH_SE1; // row vector of scl_AH_SE1i for i = 1,2,...10.
  row_vector [9] scl_AH_SE2; // row vector of scl_AH_SE2i for i = 1,2,...10.
  ...
  // Weather date to exclude: scl_AH = [scl_AH1,scl_AH2,scl_AH3,scl_AH4,scl_AH5,scl_AH6,scl_AH7,scl_AH8,scl_AH9];
  ...
  for (t in 1:TT){
    // Weather date to exclude: mean_R[t] = gama +  coef_NY * EXPVAR[1,t] + coef_AH * EXPVAR[2,t] + scl * EXPVAR[3:11,t] + scl_AH * EXPVAR[12:20,t] + coef_SE1 * EXPVAR[21,t] + coef_SE2 * EXPVAR[22,t] +  coef_pre_SE1 * EXPVAR[23,t] + scl_AH_SE1 * EXPVAR[24:32,t] + scl_AH_SE2 * EXPVAR[33:41,t] + scl_AH_pre_SE1 * EXPVAR[42:50,t];
    mean_R[t] = gama +  coef_NY * EXPVAR[1,t] + scl * EXPVAR[2:10,t] + coef_SE1 * EXPVAR[11,t] + coef_SE2 * EXPVAR[12,t] +  coef_pre_SE1 * EXPVAR[13,t] + scl_AH_SE1 * EXPVAR[14:22,t] + scl_AH_SE2 * EXPVAR[23:31,t] + scl_AH_pre_SE1 * EXPVAR[32:40,t];
  }
